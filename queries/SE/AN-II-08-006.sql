SELECT filter.KODE_KAB, filter.KODE_KEC, filter.NAMA_KEC, 
    ...
    filter.nama_usaha, filter.jenis_usaha_value, filter.jenis_kawasan_value
...
FROM (
    ...
    INNER JOIN USAHA_REF u
        ON a.assignment_id = u.assignment_id
            AND a.assignment_date_modified = u.assignment_date_modified
    WHERE jenis_usaha_value = 2 AND 
      jenis_kawasan_value <> 10
) AS filter
